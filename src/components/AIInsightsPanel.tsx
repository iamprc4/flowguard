...
export default function AIInsightsPanel({ selectedEventTitle, selectedEventLocation }: AIProps) {
  const [messages, setMessages] = useState<Array<{ sender: 'ai' | 'user'; text: string; timestamp: Date }>>([
    {
      sender: 'ai',
      text: `Hello, Commander. I am the FlowGuard AI Congestion Advisor, trained on 8,173 real Bengaluru traffic incidents.

Based on the live dataset, here are my active strategic observations:
• **Barricade Alert:** Silk Board Junction has recorded 33 incidents this cycle and is approaching critical density during evening hours. Recommend deploying hard barricades on Hosur Road approach by 17:00 IST.
• **Manpower Advisory:** Mekhri Circle is the highest-risk junction in the dataset with 64 recorded incidents — pre-position 6 officers here before any evening event above 10,000 attendees.
• **Peak Hour Warning:** Incident volume peaks at 21:00 IST across Bengaluru — all deployments should be active by 20:00 IST.

Type any tactical or scheduling question below to generate AI-powered Bengaluru traffic advice.`,
      timestamp: new Date()
    }
  ]);
      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-4 bg-slate-950/40">
        {messages.map((m, idx) => (
          ...
          >
            <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs shrink-0 ${
              m.sender === 'user' ? 'bg-[#7B61FF] text-white' : 'bg-cyan-500/10 border border-cyan-500/20 text-cyan-400'
            }`}>
              {m.sender === 'user' ? 'U' : <Terminal className="w-3.5 h-3.5" />}
            </div>
            
            <div className={`rounded-xl p-3.5 text-xs inline-block shadow-lg ${
              m.sender === 'user' 
        ...
        {loading && (
          <div className="flex gap-3 max-w-[80%] mr-auto items-center">
            ...
            <span className="text-[10px] font-mono text-gray-500 uppercase tracking-widest">Generating dispatcher matrices...</span>
          </div>
        )}
      </div>
